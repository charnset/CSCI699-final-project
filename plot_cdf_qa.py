...
def extract_dir(directory):
	...
	ax.plot(cdf["SQuAD"]['x'], cdf["SQuAD"]['y'], linewidth=4, linestyle="-", color="#39ff14", label="SQuAD")

	for label in in_domain:
	...
	ax.plot(cdf["BENIGN"]['x'], cdf["BENIGN"]['y'], linestyle="--", color="#111111", label="BENIGN")

	ax.set_xlabel("Maximum Softmax Probability", fontsize=12)
	ax.set_ylabel("Fraction of Data", fontsize=12)
	ax.set_title("CDF of MSP for Queries on BERT pre-trained on SQuAD", fontsize=14)
