...
def create_dataset(query_data, sampling_size):
    ...
    def encode(query):
        '''
        tokenized_context = tokenizer(query["context"], padding="max_length", truncation=True, max_length=128, return_tensors="pt")
        tokenized_question = tokenizer(query["question"], padding="max_length", truncation=True, max_length=128, return_tensors="pt")
        encoded_query = {}
        
        encoded_query["input_ids"] = np.concatenate((tokenized_context["input_ids"], tokenized_question["input_ids"]), axis=1)
        encoded_query["attention_mask"] = np.concatenate((tokenized_context["attention_mask"], tokenized_question["attention_mask"]), axis=1)
        '''
...
def extract_dir(training_file, directory):
    testing_file_s = [os.path.join(directory, f) for f in os.listdir(directory) if ".gz" in f]
    for testing_file in testing_file_s:
        data_x = read_data(training_file)
        data_y = read_data(testing_file)
...
if __name__ == "__main__":
    ...
    extract_dir(args.b, args.i) #in-domain
    logger.info('-'*20 + "OUT-DOMAIN DATASETS" + '-'*20)
    extract_dir(args.b, args.o) #out-domain
...
data_x = read_data("in_domain_dev/SQuAD.jsonl.gz")
data_y = read_data("in_domain_dev/NewsQA.jsonl.gz")
logger.info("read data")
#logger.info("{}".format(len(data_x)))
#logger.info("{}".format(len(data_y)))
logger.info("{}".format(len(data_x["question"])))
logger.info("{}".format(len(data_y["question"])))
...
logger.info("mmd")
logger.info("{}".format(MMD(dataset_x, dataset_x, "rbf")))
logger.info("{}".format(MMD(dataset_x, dataset_y, "rbf")))
